...
function axkvaMin:resetInstance(pPlayer)
  ...
  writeData("axkvaMin:occupiedState", 0)
  -- H(am): Lev's handleVictory (axkvaMin.lua:1066-1076 on SD1) clears these three keys
  -- on a win. resetInstance -- the timeout path (checkIfActive :971-978) and the
  -- EXITEDBUILDING path (resetInstanceA :1025-1035) -- cleared only occupiedState, so
  -- after any non-victory end bossOneSpawnState stayed 1 and spawnBossOneCheck
  -- (:184-191) returned early forever: the instance reopened with no boss 1 and could
  -- never be completed again until a server restart. Same three keys, same values, taken
  -- from Lev's own victory path.
  writeData("axkvaMin:trashSpawnState", 0)
  writeData("axkvaMin:bossOneSpawnState", 0)
  writeData("axkvaMin:bossOneTrashState", 0)
end
