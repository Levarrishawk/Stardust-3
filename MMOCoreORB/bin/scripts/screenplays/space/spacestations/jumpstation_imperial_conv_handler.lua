...
-- string/en/conversation/battlefield_entry_station_imperial.stf - see the key map
-- at the head of mobile/conversations/space/jumpstation_imperial.lua.
--
...
-- used here is:
--   s_33077605 "You are not welcome at this station, Rebel SCUM!"    -> overt Rebel
--   s_33f71018 "You are not of the proper faction..."                -> Rebel, not overt
--   s_2e491159 "Sign up with the Empire and we'll talk."             -> no faction
-- Imperial players are never gated on faction status, so no declared Imperial can
-- be locked out by this mapping.

local Logger = require("utils.logger")
require("utils.helpers")

JumpstationImperialConvoHandler = conv_handler:new {}

function JumpstationImperialConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	if (faction == FACTIONREBEL) then
		if (CreatureObject(pPlayer):getFactionStatus() > ONLEAVE) then
			return convoTemplate:getScreen("jumpstation_imperial_enemy")
		end

		return convoTemplate:getScreen("jumpstation_imperial_wrong_faction")
	elseif (faction ~= FACTIONIMPERIAL) then
		return convoTemplate:getScreen("jumpstation_imperial_sign_up")
	end

	return convoTemplate:getScreen("jumpstation_imperial_greeting")
end

function JumpstationImperialConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	-- s_fe2e6ec0 instead, which also carries %DI.
	if (screenID == "jumpstation_imperial_deepspace") then
		local prestigeType = DeepSpaceJumpScreenPlay:getPrestigeType(pPlayer, "imperial")
		local balance = DeepSpaceJumpScreenPlay:getPrestigeBalance(pPlayer, prestigeType)

		if (prestigeType == nil or balance < cost) then
			local pInsufficient = LuaConversationTemplate(pConvTemplate):getScreen("jumpstation_imperial_insufficient")
	...
	if (screenID == "jumpstation_imperial_deepspace_jump") then
		createEvent(3500, "DeepSpaceJumpScreenPlay", "beginJump", pPlayer, "imperial")
	end
	...
	if (screenID == "jumpstation_imperial_kessel_jump") then
