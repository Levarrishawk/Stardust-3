...
-- The Heavy X-wing handed over by recovery/ep3_heavy_xwing_recovery.
-- Body copied verbatim from player_xwing.lua; only the two values below differ.
object_ship_player_player_advanced_xwing = object_ship_player_shared_player_advanced_xwing:new {

	-- CHASSIS ALIAS: ShipManager::getChassisData() looks the chassis row up by this string, and the
	-- client's datatables/space/ship_chassis.iff has no "player_advanced_xwing" row -- the only
	-- X-wing player row it ships is "player_xwing". So this hull borrows that row, which is the
	-- engine's designed mechanism for exactly this case: player_yt1300_decorated_01.lua borrows
	-- "player_yt1300" the same way and is likewise absent from the client table.
	name = "player_xwing",
	slideFactor = 1.95,
	chassisHitpoints = 6000,
	-- AUTHORED. station_rebel_outpost.stf s_815 only says this is "a heavy version of the X-wing
	-- fighter. It has much greater mass capacity than the standard one!" -- qualitative, no number.
	-- Sibling-matched to the one in-tree family that has both a medium and a heavy player hull:
	-- player_blacksun_medium_s01.lua chassisMass = 95000 -> player_blacksun_heavy_s01.lua
	-- chassisMass = 190000, exactly x2.0. Applied to the standard X-wing's 100000 that gives
	-- 200000. Category/level/hitpoints/components are left as player_xwing.lua has them, because
	-- the client says nothing about them.
	chassisMass = 200000,
