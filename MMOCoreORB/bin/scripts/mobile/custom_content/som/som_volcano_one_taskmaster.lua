-- Volcano Battlefield event_one boss, the Droid Army Taskmaster.
-- Live row som_volcano_one_taskmaster, level 85 BOSS; level here is APEX tier 140, not live's 85 -- see
-- the note below.
...
-- mustafar/mustafar_trial_taskmaster and its chronicle relic are absent, so the hk77 loot block
-- stands in. Live setHp of 545000 is dropped for the APEX tier's baseHAM/baseHAMmax. Live scale
-- 1.5 is dropped because Core3 Creature templates have no scale field.
...
-- from live.
som_volcano_one_taskmaster = Creature:new {
	customName = "a Droid Army Taskmaster",
...
CreatureTemplates:addCreatureTemplate(som_volcano_one_taskmaster, "som_volcano_one_taskmaster")
